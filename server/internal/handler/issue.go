...
func (h *Handler) UpdateIssue(w http.ResponseWriter, r *http.Request) {
	...
	if statusChanged && issue.Status == "blocked" && h.OverseerService != nil {
		if _, err := h.OverseerService.MaybeEscalateBlockedIssue(r.Context(), issue); err != nil {
			slog.Warn("blocked issue overseer escalation failed", append(logger.RequestAttrs(r), "error", err, "issue_id", id, "workspace_id", workspaceID)...)
		}
	}
