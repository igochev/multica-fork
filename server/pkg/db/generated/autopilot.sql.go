...
const getProjectLinkedOverseerAutopilot = `-- name: GetProjectLinkedOverseerAutopilot :one
...
func (q *Queries) GetProjectLinkedOverseerAutopilot(ctx context.Context, projectID pgtype.UUID) (Autopilot, error) {
	row := q.db.QueryRow(ctx, getProjectLinkedOverseerAutopilot, projectID)
	var i Autopilot
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
